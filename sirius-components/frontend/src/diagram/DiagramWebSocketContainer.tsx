...
//import BarChartSchedule from "diagram/BarChartSchedule";
...
export const DiagramWebSocketContainer = ({
  ...
  //Amir added
  const [showScheduleMap, setShowScheduleMap] = useState(new Map())
  const toggleShowSchedule = (key, value) => {
    setShowScheduleMap(map => new Map(map.set(key, value)))
  }
  const onShowOrHideSchedule = async () => {
    toggleShowSchedule(
      representationId, !showScheduleMap.get(representationId))

    const switchRepresentationEvent: SwithRepresentationEvent =
      { type: 'SWITCH_REPRESENTATION', representationId };
  ...
  }, [displayedRepresentationId, representationId, dispatch, showScheduleMap]);
    ...
    contextualPaletteContent = (
      <div className={classes.contextualPalette} style={style}>
        <ContextualPalette
          ...
          selectedObjectId={selection.id}
          onShowSchedule={onShowOrHideSchedule}
  ...
  const handleNoScheduling = useCallback(
    () => {
      const showToastEvent: ShowToastEvent = {
        type: 'SHOW_TOAST',
        message: 'No scheduling avaible for this node',
      };
      dispatch(showToastEvent);
    },
    [dispatch]
  ...
  let content = (
    <div id="diagram-container" className={classes.diagramContainer}>
      {showScheduleMap.get(representationId) ?
        (<ApexChart key={docuId} onHideSchedule={onShowOrHideSchedule}
          onError={handleNoScheduling}
          docuId={docuId}
          projId={editingContextId} objId={selection.id} kind={selection.kind}
          label={selection.label}></ApexChart>)
        :
        (<div id="diagram-wrapper" className={classes.diagramWrapper}>
          <div ref={diagramDomElement} id="diagram"
  ...
  if (diagramWebSocketContainer === 'complete' && !showScheduleMap) {
    content = (
