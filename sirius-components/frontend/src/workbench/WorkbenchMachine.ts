  ...
  },
  {
    actions: {
      updateSelection: assign((context, event) => {
        ...
        if(selection.docuId===undefined)
          selection.docuId = context.selection.docuId;
      
        if (isRepresentation) {
          const { id, label, kind, docuId } = selection;
          const representation: Representation = { id, label, kind, docuId };
          ...
          if (!selectedRepresentation) {
            ...
            representation.docuId = selectedRepresentation.docuId; 
          }

          console.log("in assign: ");
          console.log(representation.docuId);
